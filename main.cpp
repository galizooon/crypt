...
void check_number_flags(int argc, char *argv[], const std::vector <char> &c_fl) {
  const unsigned char modes_sum = c_fl[2] + c_fl[3] + c_fl[4] + c_fl[5] + c_fl[6] + c_fl[7]; // РЎСѓРјРјР° РєРѕР»РёС‡РµСЃС‚РІР° РІСЃС‚СЂРµС‡Р°СЋС‰РёР№С…СЃСЏ СЂРµР¶РёРјРѕРІ,
  const unsigned char help_sum = c_fl[0] + c_fl[1];                                          // С„Р»Р°РіРѕРІ РЅР° РїРѕРјРѕС‰СЊ,
  unsigned char count_flags_sum = 0;                                                   // РІСЃРµС… С„Р»Р°РіРѕРІ
...
void check_enc_dec(int argc, char *argv[], const std::vector <char> &c_fl) {
  if (((c_fl[8] + c_fl[9]) == 0) && (!c_fl[7])) {
    throw "Выберите режим enc или dec!\n";
...
//	const std::string& filename = "parole.key";
	...
	std::cout<< "1. Cоздание ключевого файла с помощью ГПСЧ пропущенный через хэш-функцию ГОСТ 34.11"<<std::endl;
	std::cout<< "2. Cоздание ключевого файла на основе пароля"<<std::endl;
	std::cout<< "3. Зашифрование произвольного двоичного файла с использованием указанного ключевого файла, с помещением результата в некий файл-контейнер "<<std::endl;
	std::cout<< "4. Расшифрование произвольного двоичного файла с использованием указанного ключевого файла, с извлечением его из файла-контейнера"<<std::endl;
	std::cout<< "Выбор:"<<std::endl;

	std::cin>> select_operation;

if (select_operation == 1){
	  std::cout<< "Введите имя ключевого файла"<<std::endl;
	  std::cin >> th_filename;
	  std::cout<< "Введите размер ключа"<<std::endl;
	  std::cin >> k_size;
...
//test(text);
	...
	std::cout<<"Введите название зашифрованного файла"<<std::endl;
	std::string cfile;
	std::cin>>cfile;
	...
	std::cout << "1. Режим замены (ECB)"<< std::endl;
	std::cout << "2. Режим гаммирования (XOR)"<< std::endl;
	std::cout << "3. Cцепление блоков по шифротексту (CBC)"<< std::endl;
	std::cin>>mode;
...
//		cbc(c_fl, text, ctext, etext, key, argv, argc);
	...
	std::cout<<"Введите название файла, который хотите зашифровать"<<std::endl;
		...
		std::cin>>efile;
	std::cout << "Выберите режим:"<< std::endl;
		std::cout << "1. Режим простой замены (ECB)"<< std::endl;
		std::cout << "2. Режим гаммирования (CTR)"<< std::endl;
		std::cout << "3. Cцепление блоков по шифротексту (CBC)"<< std::endl;
		std::cin>>mode;
